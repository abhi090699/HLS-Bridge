...
`ifdef DTI_TB_IN_PASSIVE_MODE
        ...
        begin
          int l_p_req_group  = 0 * parameters_cfg_pkg::LBB_NUM_TLP_STREAMS + l_stream;
          int l_p_resp_group = 2 * parameters_cfg_pkg::LBB_NUM_TLP_STREAMS + l_stream;
          int            l_dti_ends;
          int            l_ends_per_beat;
          int unsigned   l_beats;
          time           l_clk_period = 0;
          l_ends_per_beat = $countones(hls_ib_posted_hal_cxs_pkt.CurEndedPktTx);
          if (l_ends_per_beat == 0)
            l_ends_per_beat = 1;
          if (hls_ib_posted_hal_cxs_pkt.Length > 0)
            l_beats = int'(hls_ib_posted_hal_cxs_pkt.Length);
          else if (hls_ib_posted_hal_cxs_pkt.DataPerPktTx.size() > 0)
            l_beats = hls_ib_posted_hal_cxs_pkt.DataPerPktTx.size();
          else
            l_beats = 1;
          m_env_cfg.m_misc_if_api.get_core_clk_period(l_clk_period);
          if (l_beats == 1 && l_ends_per_beat == 1 && l_clk_period > 0 &&
              hls_ib_posted_hal_cxs_pkt.PktEndTime > (hls_ib_posted_hal_cxs_pkt.PktStartTime + l_clk_period))
            l_dti_ends = 2;
          else
            l_dti_ends = l_ends_per_beat * l_beats;
          if (l_dti_ends < 1)
            l_dti_ends = 1;
          m_qos_expected_count[l_p_req_group]  += l_dti_ends;
          m_qos_expected_count[l_p_resp_group] += l_dti_ends;
          m_qos_expected_count[l_p_resp_group]++;
          `uvm_info("QOS_EXP_DTI", $sformatf(
            "POSTED DTI: stream=%0d P&REQ group=%0d expected=%0d P&RESP group=%0d expected=%0d (dti_pck_ends=%0d)",
            l_stream, l_p_req_group, m_qos_expected_count[l_p_req_group],
            l_p_resp_group, m_qos_expected_count[l_p_resp_group], l_dti_ends), UVM_DEBUG)
        end
      end
`endif
        ...
        begin
          int l_group = 2 * parameters_cfg_pkg::LBB_NUM_TLP_STREAMS + l_stream;
          m_qos_expected_count[l_group]++;
          `uvm_info("QOS_EXP_AXI", $sformatf(
            "POSTED: port=%0d stream=%0d group=%0d (P&RESP) expected=%0d",
  ...
  virtual task process_hls_ib_nonposted_hal_pkt_ended(string msg_id = "");
    ...
    forever begin
	 ...
	 begin
 		 ...
 		 int l_group  = 3 * parameters_cfg_pkg::LBB_NUM_TLP_STREAMS + l_stream;
 		 m_qos_expected_count[l_group]++;
 		 `uvm_info("QOS_EXP_AXI", $sformatf(
 		   "NONPOSTED AXI: port=%0d stream=%0d group=%0d (NP&RESP) expected=%0d",
 		   l_hls_port_num, l_stream, l_group, m_qos_expected_count[l_group]), UVM_DEBUG)
	 end
      end
`ifdef DTI_TB_IN_PASSIVE_MODE
	...
	begin
  		int l_stream = int'(hls_ib_nonposted_hal_tlp_pkt.hls_ib_p_np_meta_s.idgroup[2:0]);
  		int l_np_req_group  = 1 * parameters_cfg_pkg::LBB_NUM_TLP_STREAMS + l_stream;
  		int l_np_resp_group = 3 * parameters_cfg_pkg::LBB_NUM_TLP_STREAMS + l_stream;
  		int            l_dti_ends;
  		int            l_ends_per_beat;
  		int unsigned   l_beats;
  		time           l_clk_period = 0;
  		l_ends_per_beat = $countones(hls_ib_nonposted_hal_cxs_pkt.CurEndedPktTx);
  		if (l_ends_per_beat == 0)
  		  l_ends_per_beat = 1;
  		if (hls_ib_nonposted_hal_cxs_pkt.Length > 0)
  		  l_beats = int'(hls_ib_nonposted_hal_cxs_pkt.Length);
  		else if (hls_ib_nonposted_hal_cxs_pkt.DataPerPktTx.size() > 0)
  		  l_beats = hls_ib_nonposted_hal_cxs_pkt.DataPerPktTx.size();
  		else
  		  l_beats = 1;
  		m_env_cfg.m_misc_if_api.get_core_clk_period(l_clk_period);
  		if (l_beats == 1 && l_ends_per_beat == 1 && l_clk_period > 0 &&
  		    hls_ib_nonposted_hal_cxs_pkt.PktEndTime > (hls_ib_nonposted_hal_cxs_pkt.PktStartTime + l_clk_period))
  		  l_dti_ends = 2;
  		else
  		  l_dti_ends = l_ends_per_beat * l_beats;
  		if (l_dti_ends < 1)
  		  l_dti_ends = 1;
  		m_qos_expected_count[l_np_req_group]  += l_dti_ends;
  		m_qos_expected_count[l_np_resp_group] += l_dti_ends;
  		m_qos_expected_count[l_np_resp_group]++;
  		`uvm_info("QOS_EXP_DTI", $sformatf(
  		  "NONPOSTED DTI: stream=%0d NP&REQ group=%0d expected=%0d NP&RESP group=%0d expected=%0d (dti_pck_ends=%0d)",
  		  l_stream, l_np_req_group, m_qos_expected_count[l_np_req_group],
  		  l_np_resp_group, m_qos_expected_count[l_np_resp_group], l_dti_ends), UVM_DEBUG)
	end
      end
`endif
      //-Tracking IB packet sequence
  ...
  virtual task process_hls_ib_compl_hal_pkt_ended(string msg_id = "");
	...
	begin
  		int l_group = 1 * parameters_cfg_pkg::LBB_NUM_TLP_STREAMS + int'(l_hls_port_num[2:0]);
  	        m_qos_expected_count[l_group]++;
		`uvm_info("QOS_EXP",$sformatf("COMPL AXI: port=%0d group=%0d (NP&REQ) expected=%0d",l_hls_port_num, l_group,m_qos_expected_count[l_group]),UVM_DEBUG)
	end
      end
