# Flit-mode TLP fields: Memory Request, Completion, and CFG (with optional OHC)
...
## 5. Side-by-side: what moved vs Non-Flit
...
## 6. HLS Bridge decode notes

Inbound HLS packets must expose the fields above for P/NP/C arbitration (`hls_brg_ib_*`). Practical decode order:

1. DW0 OHC[4:0] and TS — size of optional header + trailer.
2. Type: Mem vs CFG vs Cpl.
...
6. If OHC-C and Sub-stream != `4'h7`: IDE TLP (Stream ID, K, T).
7. Completer metadata `REQ_SEG_IN_TLP_IS_VALID` applies only in Flit Mode: when 0, the controller inserts captured segment into OHC-A5 Completer Segment (Cpl) or OHC-C RSV (others).
